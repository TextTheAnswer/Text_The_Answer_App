...
class QuizBloc extends Bloc<QuizEvent, QuizState> {
  ...
  QuizBloc({ApiService? apiService})
    ...
    on<FetchDailyQuiz>((event, emit) async {
      emit(QuizLoading());
      try {
        printDebug('QuizBloc: Fetching daily quiz...');
        ...
        printDebug(
          'QuizBloc: Successfully loaded ${questionsList.length} questions',
        );
        ...
        printDebug('QuizBloc Error (FetchDailyQuiz): $e'); // Debug statement
    ...
    on<SubmitQuizAnswer>((event, emit) async {
      ...
      try {
        printDebug(
          'QuizBloc: Submitting answer "${event.answer}" for question ${event.questionId}',
        );
        ...
        printDebug(
          'QuizBloc: Answer is ${isCorrect ? "correct" : "incorrect"}, Points: $points',
        );
        ...
        try {
          ...
          printDebug('QuizBloc: API response received for answer submission');
          ...
          emit(
            QuizAnswerSubmitted(
              ...
              explanation:
                  ...
                  (isCorrect
                      ? 'Correct!'
                      : 'The correct answer was: ${acceptedAnswers.first}'),
              questionsAnswered:
                  response['questionsAnswered'] ??
                  (currentState.questionsAnswered + 1),
              correctAnswers:
          ...
          printDebug(
            'QuizBloc: API error submitting answer: $apiError, using local validation',
          );
          ...
          emit(
            QuizAnswerSubmitted(
              ...
              streak:
                  isCorrect
                      ? (currentState is QuizAnswerSubmitted
                          ? (currentState as QuizAnswerSubmitted).streak + 1
                          : 1)
                      : 0,
              points: points,
              totalPoints:
        ...
        printDebug('QuizBloc Error (SubmitQuizAnswer): $e');
    ...
    on<SubmitQuizAnswersBulk>((event, emit) async {
      ...
      try {
        printDebug(
          'QuizBloc: Submitting ${event.answers.length} answers in bulk',
        );
        ...
        final answersWithTimeSpent = QuizHelper.convertToTimeSpent(
          event.answers,
        );

        // Filter out invalid question IDs before submission to prevent ObjectId casting errors
        final validAnswers = QuizHelper.filterValidQuestionIds(
          answersWithTimeSpent,
        );
        ...
        if (validAnswers.isEmpty) {
          emit(
            QuizError(
              message:
                  'No valid answers to submit. All question IDs were invalid.',
            ),
          );
          return;
        }
        ...
        if (validAnswers.length < answersWithTimeSpent.length) {
          printDebug(
            'QuizBloc: Filtered out ${answersWithTimeSpent.length - validAnswers.length} invalid question IDs',
          );
        }
        ...
        if (response['success'] == true || response.containsKey('results')) {
          ...
          printDebug('QuizBloc: Bulk submission successful');
          ...
          emit(
            QuizError(
              message: response['message'] ?? 'Failed to submit bulk answers',
            ),
          );
        }
      } catch (e) {
        printDebug('QuizBloc Error (SubmitQuizAnswersBulk): $e');
        emit(
          QuizError(message: 'Failed to submit bulk answers: ${e.toString()}'),
        );
      }
    });
  }
